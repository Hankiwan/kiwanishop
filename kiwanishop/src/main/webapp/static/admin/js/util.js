...
//개체의 갯수 리턴 : boxCnt(document.frm,document.frm.bx,'bx')
function boxCnt(f,bx,ename) {
	var bxcount = 0;
...
//체크 박스가 한개도 없을수도 있을때 체크된 갯수 리턴 : selectboxCheckCnt(document.frm,document.frm.bx,'bx')
...
//SNS 연동
function fncSnsConnect(gubun, dateChk ,type) {
	
	var WEB_PATH_JS_SNS = "/card2012";
	
	var linkUrl = "";
	if(type == "ECOMMON" || type == "CEVENT"){
		linkUrl = "http://www.oilbankcard.com"+WEB_PATH_JS_SNS+"/front/event_detail.do?cbcode="+type+"&cecode="+dateChk;
	}else{
		linkUrl = "http://www.oilbankcard.com"+WEB_PATH_JS_SNS+"/front/boardView.do?bcode=EPAST&currPage=&page_num=150&pre_page_num=150&cecode="+dateChk;
	}
	var url ="";	
	linkUrl = encodeURIComponent(linkUrl);	//링크 url
		if(gubun=="facebook"){
			url = "http://www.facebook.com/sharer.php?u="+linkUrl;
		}else if(gubun=="twitter"){
			url = "http://twitter.com/home?status="+linkUrl;	
		}else{
			url = "http://me2day.net/posts/new?new_post[body]="+linkUrl;
		}	
		window.open(url,'','');
	}
